...
@login_required
def cart(request):
    items = Transaction.objects.filter(buyer=request.user.profile, status='on_cart')
    context = {'transactions': items}
    return render(request, 'merchstore/cart.html', context)

@login_required
def transactions(request):
    items = Transaction.objects.filter(product__owner=request.user.profile).exclude(status='on_cart')
    context = {'transactions': items}
